    ...
    public static void main(String[] args) {
        Thread checkMax = new Thread(() -> {
            ...
            while (!Thread.interrupted()) {
                synchronized (sizeToFreq) {
                    ...
                    for (int cnt : sizeToFreq.keySet())
                        if (sizeToFreq.get(cnt) > max) {
                            max = sizeToFreq.get(cnt);
                            keyMax = cnt;
                        }
                    System.out.println("Текущий лидер среди частот: " + keyMax + " - " + max);
                }
            }
        });
        checkMax.start();
        ...
        for (int i = 0; i < 100; i++) {
            threadList.add(new Thread(() -> {
                ...
                System.out.println("Число поворотов направо: " + count);
                synchronized (sizeToFreq) {
        ...
        while (checkMax.isAlive()) checkMax.interrupt();
